...
This lab walks you through setting up [**Git**](./glossary.md#git) and **VS Code**, then using their graphical interfaces to manage the `class-cp115-repo` on [**GitHub**](./glossary.md#github).
...
- In **VS Code**, open the `Extensions` panel and install the **Python** [extension](./glossary.md#extension) from **Microsoft**.
...
[**GitHub CLI**](./glossary.md#github-cli) (`gh`) is a command-line tool that brings **GitHub** directly into your [terminal](./glossary.md#terminal). In this course, we use it to authenticate your machine with **GitHub** so that pushing and pulling code works without entering your credentials every time.
...
Install **GitHub CLI** using [`winget`](./glossary.md#winget):
...
Run the following [command](./glossary.md#command) to authenticate with **GitHub**:
...
When you [fork](./glossary.md#fork) a [repository](./glossary.md#repository) on **GitHub**, you create a fully independent copy under your own account. The original owner keeps their version; you now control yours and can set permissions, open issues, or adjust settings without affecting the source project.
...
To solve this, we need to [clone](./glossary.md#clone) the repository into your computer so you can edit and run the code using your [IDE](./glossary.md#ide), which in this class we have chosen **VS Code**.
...
Back in **VS Code**, choose [`Source Control`](./glossary.md#source-control) from the sidebar and click `Clone repository` and paste the link that you have copied in the box when prompted. Then choose `Clone from URL`.
...
But what exactly is `Commit`? A [commit](./glossary.md#commit) records a snapshot of your changes in the local repository. Each commit carries a unique ID and a message explaining what changed, letting you roll back or review history at any point.

[`Push`](./glossary.md#push) is the **Git** action that copies your new commits from the local repository on your computer to the remote repository, which in our case is `cp115-class-repo`. Although the button shows `Sync`, behind the scene what it will do is `Push` and `Pull`.
...
Once you fork a project, your copy starts aging the moment the original (“up-stream”) repo receives new commits. [Syncing a fork](./glossary.md#sync-fork) is the act of pulling those new commits into your fork so it stays current—and so your pull requests don’t conflict with code that has already moved on.

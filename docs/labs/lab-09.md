...
## Exercise 5: Health Assessment Report <Badge type="warning" text="Task" />

Generate a comprehensive health assessment with multiple warning flags and recommendations.

A patient visits a clinic. Based on their vital signs and lifestyle factors, identify all applicable health warnings and calculate their overall risk score.

**Assessment Criteria:**
- BMI = weight / (height²)
- Blood pressure risk if systolic > 140 OR diastolic > 90
- Heart rate concern if < 60 OR > 100
- Cholesterol warning if total > 200
- Age factor applies if over 50
- Smoking adds +20 to risk score
- Exercise < 3 hours/week adds +15 to risk score  
- Family history adds +10 to risk score
...
## Exercise 6: Event Planning Budget <Badge type="warning" text="Task" />

Calculate the total cost for organizing an event with various optional services and requirements.

An event planner needs to budget for a corporate event. Different requirements add costs, and multiple special conditions affect the final pricing.

**Cost Factors:**
- Base venue cost: RM50 per person
- Weekend events add RM15 per person
- Evening events (after 6pm) add RM12 per person  
- Catering upgrade: RM25 per person
- Audio/Visual equipment: RM800 flat fee
- Security required if guests > 100: RM500 flat fee
- Parking needed if venue has no parking: RM300 flat fee
- Insurance required if alcohol served: RM200 flat fee
- Cleanup fee if outdoor event: RM400 flat fee
...
## Exercise 7: Smart Home Energy Monitor <Badge type="warning" text="Task" />

Analyze household energy consumption and provide efficiency recommendations.

A smart home system monitors various devices and calculates energy costs. Multiple efficiency recommendations are generated based on usage patterns and device conditions.

**Energy Analysis:**
- Base electricity rate: RM0.45 per kWh
- Peak hours (7-11pm): +50% rate surcharge
- Air conditioning efficiency warning if usage > 8 hours AND temp < 22°C
- Water heater alert if always-on mode detected
- Lighting recommendation if average > 6 hours daily
- Appliance age concern if any device > 10 years old
- High consumption alert if monthly total > 600 kWh
- Eco bonus: -10% if solar panels AND usage < 400 kWh
- Time-of-use savings available if peak usage > 40% of total
